...
def get_payoffs(payoff_type,dh_spacing=0.05,alpha_list = [0.1],user_p1_payoff=[[0,1],[0.2,0.6]],user_p2_payoff=[[0,0.2],[1,0.6]],dh_list = [0.5], dd_list = [0.75]):
        ...
        for dh in payoff_dh:
            for dd in payoff_dd:
                if dh < dd and dh >= 0.1 and dd <= 0.9:
                    ...
                    p1_payoffs_list_in.append(this_payoff_p1)
                    p2_payoffs_list_in.append(this_payoff_p2)
    elif payoff_type == "RandSym": # Symmetric payoff space
        payoff_dd = np.random.random(400)
        payoff_dh = np.random.random(400)
        for dh,dd in zip(payoff_dh,payoff_dd):
            if dh < dd - 0.01 and dh >= 0.1 and dd <= 0.9:
                this_payoff_p1 = np.array([[0,1],[np.round(dh,2),np.round(dd,2)]])
                this_payoff_p2 = np.array([[0,np.round(dh,2)],[1,np.round(dd,2)]])
                p1_payoffs_list_in.append(this_payoff_p1)
                p2_payoffs_list_in.append(this_payoff_p2)
